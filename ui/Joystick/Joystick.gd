...
func _input(event: InputEvent) -> void:
	...
	if output.y < -0.2:
		Input.action_press("ui_up", -output.y)
	else:
		Input.action_release("ui_up")
	if output.x > 0.2:
		Input.action_press("ui_right", output.x)
	elif output.x < -0.2:
		Input.action_press("ui_left", -output.x)
	#elif not(Input.is_action_pressed("ui_right") or Input.is_action_pressed("ui_left")):
	else:
